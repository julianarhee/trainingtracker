...
def find_repeats(L, nreps):
    ...
    counts = 0
    while idx < len(L):
        if L[idx] != 0 and [L[idx]]*nreps == L[idx:idx+nreps]:
            L[idx:idx+nreps] = [True]*nreps
            idx += nreps
            counts += 1
        else:
            idx += 1
...
def get_incrementing(evs, test=lambda a, b: b.value != (a.value - 1),
                   recurse=True):
    """
    ...
    if recurse and len(bad_indices):
        bw = get_incrementing(get_incrementing(events, test=test, recurse=recurse))
...
if __name__ == '__main__':
    ...
    for input_path in input_paths:
        ...
        # super-dict of all animals (input-paths), contains sub-dicts:
        # data = { {'input/animal/animal_date': stats-dict}, {'.../animal_date':DICT}, ..., {} }
        ...
        # data = process_datafiles(dfns, stats) 
            ...
            if not ((tmp_correct_licks+tmp_bad_ignores == tmp_nTargets) \
                ...
                if (tmp_correct_licks+tmp_bad_ignores) != tmp_nTargets:
                    print 'Target count is %d: OFF by %d, in %s' % (tmp_nTargets,tmp_nTargets-targ_outcomes,fn)
                    print 'CORR-LICKS %d + BAD-IGNS %d = %d.' % (tmp_correct_licks, tmp_bad_ignores, targ_outcomes)
                if (tmp_correct_ignores+tmp_bad_licks) != tmp_nDistractors:
                    print 'Distractor count is %d: OFF by %d, in %s' % (tmp_nDistractors, tmp_nDistractors-dist_outcomes,fn)
                    print 'CORR-IGNS %d + BAD-LICKS %d = %d.' % (tmp_correct_ignores, tmp_bad_licks, dist_outcomes)
            # else:
            #     print "Sanity checks for stimulus-outcomes completed."
    ...
    for n in range(nSubjects):
        ...
        targprob_lowerbound = []
        for s in sessions:
            correct_lick = float(sdata[animal][s]['correct_lick'])
            correct_ignore = float(sdata[animal][s]['correct_ignore'])
            bad_lick = float(sdata[animal][s]['bad_lick'])
            bad_ignore = float(sdata[animal][s]['bad_ignore'])
