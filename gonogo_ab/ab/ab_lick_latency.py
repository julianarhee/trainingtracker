...
if __name__ == '__main__':
    ...
    for input_path in input_paths:
        ...
        for fidx, fn in enumerate(dfns):
            ...
            # print "dtype, lick_latency: ", type(lick_latency)
            # print "dtype, avg_latency: ", type(avg_latency)

            # stdev_calc = stdev(lick_latency, avg_latency)
            # if stdev != stdev_calc:
            ...
            if stderr != stderr_calc:
                print "STDERR Scipy function and calculation off: ", stderr, stderr_calc
        ...
        avg_across_sessions = np.mean(avg_latencies_session)
    ...
    for i in range(len(animal_list)):
        ax.axis([0, xvals[i][-1]+2, 0., 0.75])
        # plt.axis([1, nsessions[i], 0., 1.0])
        ax.set_ylabel('latency from stim onset (sec)', fontsize=12)
