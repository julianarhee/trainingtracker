...
def get_session_stats():
    ...
    for n in ('session_correct_lick', 'session_bad_lick', 'session_bad_ignore', \
        ...
        stats[n] = stat(functools.partial(get_session_range, name=n)) # default partial func for stat() \
                # is get_incremented_range()...
...
if __name__ == '__main__':
    ...
    for input_path in input_paths:
        ...
        countkeys = [s for s in stats.keys() if 'bad' in s or 'correct' in s \
                or 'count' in s]
        # print countkeys
        for k in countkeys:
            if 'bad_lick' in k:
