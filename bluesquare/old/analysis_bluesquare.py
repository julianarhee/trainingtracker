...
def session_incremented(evs, test=lambda a, b: b.value != (a.value - 1),
    ...
    if recurse and len(bad_indices):
        bw = session_incremented(session_incremented(events, test=test, \
            recurse=recurse))
...
def get_stats():
    ...
    for n in ('correct_lick', 'bad_lick', 'bad_ignore', \
            'correct_ignore', 'target_trialcounter', \
            'distractor_trialcounter'):
...
if __name__ == '__main__':
    ...
    for input_path in input_paths:
        ...
        # if input_path[len(input_path)-2] == 'U' or \
        #         input_path[len(input_path)-2] == 'W':
        if input_path[len(input_path)-2] == 'U':
            stats = get_stats()
            # stats = get_session_stats()
            key_corr_lick = 'correct_lick'
            key_corr_ign = 'correct_ignore'
            key_bad_lick = 'bad_lick'
            key_bad_ign = 'bad_ignore'
            key_targs = 'target_trialcounter'
            key_dists = 'distractor_trialcounter'
        else:
            stats = get_session_stats()
            key_corr_lick = 'session_correct_lick'
            key_corr_ign = 'session_correct_ignore'
            key_bad_lick = 'session_bad_lick'
            key_bad_ign = 'session_bad_ignore'
            key_targs = 'session_target_trialcounter'
        ...
        for fidx, fn in enumerate(dfns):
            ...
            tmp_correct_licks = float(data[fn][key_corr_lick])
            tmp_correct_ignores = float(data[fn][key_corr_ign])
            tmp_bad_licks = float(data[fn][key_bad_lick])
            tmp_bad_ignores = float(data[fn][key_bad_ign])

            tmp_nTargets = float(data[fn][key_targs])
            tmp_nDistractors = float(data[fn][key_dists])
            targ_outcomes = tmp_correct_licks + tmp_bad_ignores
            dist_outcomes = tmp_correct_ignores + tmp_bad_licks

            # sanity checks:
            if not ((tmp_correct_licks+tmp_bad_ignores == tmp_nTargets) \
                    and (tmp_correct_ignores+tmp_bad_licks == tmp_nDistractors)):
                if (tmp_correct_licks+tmp_bad_ignores) != tmp_nTargets:
                    print 'Target count off by %d, in %s' % (tmp_nTargets-targ_outcomes,fn)
                    print "TARGS: ", tmp_nTargets, " OUTCOMES: ", tmp_correct_licks, tmp_bad_ignores
                    print "KEY: ", key_targs, key_corr_lick, key_bad_ign
                if (tmp_correct_ignores+tmp_bad_licks) != tmp_nDistractors:
                    print 'Distractor count off by %d, in %s' % (tmp_nDistractors-dist_outcomes,fn)
                    print "DISTS: ", tmp_nDistractors, " OUTCOMES: ", tmp_correct_ignores, tmp_bad_licks
                    print "KEY: ", key_dists, key_corr_ign, key_bad_lick
        # merge epochs if nec:
    ...
    for n in range(nSubjects):
        ...
        targprob_lowerbound = []
        for s in sessions:
            correct_lick = float(sdata[animal][s][key_corr_lick])
            correct_ignore = float(sdata[animal][s][key_corr_ign])
            bad_lick = float(sdata[animal][s][key_bad_lick])
            bad_ignore = float(sdata[animal][s][key_bad_ign])
