...
def get_object_rot(datadir, baseline):
...
def group_by_size_and_rot(datadir, baseline, show_plot=True):
...
def get_array_for_plotting(datamat, sizes, rots):
    ...
    for animal in datamat.keys():
        ...
        trace = go.Heatmap(x=x, y=y, z=z, colormap='hot', showscale=False)
        ...
        plot(fig, filename='Annotated Heatmap', height=750)
...
# datadir = sys.argv[1]
datadir = '/share/coxlab-behavior/mworks-data/three_port_morphs/pnas/input_premorphs'
baseline = True

get_object_rot(datadir, baseline)
G = group_by_size_and_rot(datadir, baseline)
print G.keys()

sizes, rots, data = create_datamat(G)


import plotly.graph_objs as go

for aidx, animal in enumerate(data.keys()):
    #animal = data.keys()[0]
    print animal

    x = sizes #rots #list(sizes)
    y = rots #sizes #list(rots)
    z = data[animal]
    ...
    # colorscale = [[0, '#3D9970'], [1, '#001f3f']]  # c?ustom colorscale
    # # trace = go.Heatmap(x=x, y=y, z=z, showscale=True, colorscale='jet')
    # trace = go.Heatmap(z=z, showscale=True, colorscale='hot')
    ...
    # # py.plot(fig, filename='Annotated Heatmap', height=750)
    # plot(fig)


    # fig1 = plt.figure(figsize=(20,10))
    # ax1 = fig1.add_subplot(2,4,aidx)
    plt.pcolor(z, cmap='hot', vmin=0.5, vmax=1.0)
    plt.title(animal)
    # x0,x1 = ax1.get_xlim()
    # y0,y1 = ax1.get_ylim()
