...
class Animal():
    def __init__(self, animalid='RAT', experiment='EXPERIMENT'): #, dst_dir='/tmp'):
...
#         self.source = session_meta['datasource'] #dfn
    ...
    def plot_stats_by_transform(self, save_dir='/tmp'):
    ...
    def plot_stats_by_morph(self, save_dir='/tmp'):
        print("... Plotting stats by transform ...")
...
def process_sessions_mp(new_sessions, session_meta,
                         dst_dir='/tmp',
                         nprocesses=1,
                         ignore_flags=None,
                         response_types=['Announce_AcquirePort1', 'Announce_AcquirePort3', 'ignore'],
    ...
    def parser(curr_sessions, session_meta, ignore_flags, response_types, outcome_types, dst_dir, create_new, out_q):
        parsed_sessions = {}
...
def process_session(session_meta, 
                    ...
                    ignore_flags=None, create_new=False):
    ...
    S.plot_stats_by_transform(save_dir=dst_dir_figures)

    if any('morph' in i for i in S.stimuli):
        S.plot_stats_by_morph(save_dir=dst_dir_figures)
